...
# VARIAVEL LINHA: var2 = '' (vazio para baixar apenas as faixas da opção escolhida)
        ...
        for dado in (lista_dados):
            ...
            while i < (len(lista_municipios_UF)):
                try:
                    ...
                    time.sleep(10)
                    continue
            dados_familia.to_excel(writer, sheet_name=('num_familias_') + var1 + dados_familia.iloc[[0]].index[0][:2])
            dados_pessoa.to_excel(writer, sheet_name=('num_pessoas_') + var1 + dados_pessoa.iloc[[0]].index[0][:2])
