...
def CECADscraping(lista_UF, lista_dados, lista_municipios):
    ...
    for uf_ibge in (lista_UF):
        runs = runs + len(lista_municipios['Código Município Completo']
            ...
            while i < (len(lista_municipios_UF)):
                try:
                    ...
                    tables_list = pd.read_html(
                        raw_data, index_col=0, thousands='.')  # indexando pela cidade
                    ...
                    dados_familia = dados_familia.append(
                        tab_familia.iloc[[0]])  # Cria o df para familia
                    dados_pessoa = dados_pessoa.append(
                        tab_pessoa.iloc[[0]])  # Cria o df para pessoa
                    i += 1
                    pbar.update(1)
                except ValueError:
                    ...
                    time.sleep(10)
                    continue
            dados_familia.to_excel(writer, sheet_name=(
                'num_familias_') + var1 + dados_familia.iloc[[0]].index[0][:2])
            dados_pessoa.to_excel(writer, sheet_name=(
                'num_pessoas_') + var1 + dados_pessoa.iloc[[0]].index[0][:2])
    writer.save()
    pbar.close()
